...
local drawing_new = Drawing.new
                ...
                utility = {
                    ...
                    is_inside_fov = function(point)
                        return (point.x - objects.fov.Position.X) ^ 2 + (point.y - objects.fov.Position.Y) ^ 2 <=
                            objects.fov.Radius ^ 2
                    end,
                    ...
                    is_part_visible = function(origin_part, part)
                        ...
                        local function run_cast(origin_pos)
                            ...
                            local raycast_result =
                                workspace:Raycast(
                                origin_pos,
                                (part.Position - origin_pos).Unit * aimsp_settings.max_dist,
                                raycast_params
                            )

                            return ((raycast_result and raycast_result.Instance) or dummy_part):IsDescendantOf(
                                part.Parent
                            )
                        end

                        local head_pos =
                            (origin_part.Position + (origin_part.CFrame.UpVector * 2) + (origin_part.CFrame.LookVector))
                        ...
                        while delay() do
                            local func, result =
                                ...
                                function()
                                    ...
                                    for idx, plr in pairs(get_players()) do
                                        local plr_char =
                                            ((aimsp_settings.loop_all_humanoids or debounces.custom_players) and plr) or
                                            plr.Character
                                        ...
                                        if
                                            aimsp_settings.team_check and not aimsp_settings.loop_all_humanoids and
                                                not debounces.custom_players
                                         then
                                            if plr.Team then
                                        ...
                                        if not utility.is_dead(plr_char) then
                                            local plr_screen =
                                                ...
                                                plr_char.HumanoidRootPart.Position +
                                                    (plr_char.HumanoidRootPart.CFrame.UpVector * 2)
                                            ) -- emulate head pos
                                            ...
                                            if plr_screen ~= -1 then
                                                ...
                                                if aimsp_settings.use_esp then
                                                    local col =
                                                        (aimsp_settings.use_rainbow and utility.get_rainbow()) or white
                                                    ...
                                                    if aimsp_settings.tracers then
                                                        utility.update_drawing(
                                                            ...
                                                            plr_char:GetDebugId(),
                                                            {
                                                                ...
                                                                Thickness = 1,
                                                                Color = (aimsp_settings.use_rainbow and
                                                                    utility.get_rainbow()) or
                                                    ...
                                                    if aimsp_settings.box then
                                                        local corners =
                                                            utility.get_part_corners(plr_char.HumanoidRootPart)
                                                        ...
                                                        if
                                                            (point_a_scr ~= -1) and (point_b_scr ~= -1) and
                                                                (point_c_scr ~= -1) and
                                                                (point_d_scr ~= -1)
                                                         then
                                                            utility.update_drawing(
                                                    ...
                                                    if aimsp_settings.dist then
                                                        plr_info =
                                                            plr_info .. (tostring(math_floor(mag)) .. " Studs Away\n")
                                                    end
                                                    ...
                                                    if plr_info ~= "" then
                                                        local cam_mag =
                                                            ...
                                                            plr_char.HumanoidRootPart.CFrame.Position).Magnitude / 20

                                                        local scr_pos =
                                                            utility.to_screen(
                                                            plr_char.HumanoidRootPart.Position + vector3_new(0, 4, 0) +
                                                ...
                                                if aimsp_settings.prefer.looking_at_you then
                                                    local look_vector =
                                                        plr_char.HumanoidRootPart.Position +
                                                        (plr_char.HumanoidRootPart.CFrame.LookVector * mag)

                                                    local look_vector_lp_head_dist =
                                                        (look_vector - local_player.Character.HumanoidRootPart.Position).Magnitude
                                                    if
                                                        look_vector_lp_head_dist < dist and
                                                            utility.is_inside_fov(plr_screen)
                                                     then
                                    ...
                                    if closest_player and aimsp_settings.use_aimbot then
                                        for idx, part in pairs(closest_player:GetChildren()) do
                                            if part:IsA("BasePart") then
                                                ...
                                                if screen_pos ~= -1 then
                                                    if
                                                        utility.is_inside_fov(screen_pos) and
                                                            utility.is_part_visible(
                                        ...
                                        if lock_part then
                                            ...
                                            local top =
                                                utility.to_screen(
                                                (lock_part.obj.CFrame * cframe_new(0, scale, 0)).Position
                                            )
                                            local bottom =
                                                utility.to_screen(
                                                (lock_part.obj.CFrame * cframe_new(0, -scale, 0)).Position
                                            ...
                                            if debounces.start_aim then
                                                ...
                                                mousemoverel(
                                                    (lock_part.scr_pos.X - mouse.X) / aimsp_settings.smoothness,
                                                    (lock_part.scr_pos.Y - (mouse.Y + 36)) / aimsp_settings.smoothness
                                                )
                                            else
                                                utility.update_drawing(
                                                    objects.look_at,
