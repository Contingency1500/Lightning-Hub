...
elseif game.PlaceId == 7255828467 then
    debounces.custom_players = true
            ...
            local func, result =
                ...
                function()
                    ...
                    for idx, plr in pairs(get_players()) do
                        local plr_char =
                            ((aimsp_settings.loop_all_humanoids or debounces.custom_players) and plr) or plr.Character
                        ...
                        if not utility.is_dead(plr_char) then
                            local plr_screen =
                                utility.to_screen(
                                plr_char.LowerTorso.Position + (plr_char.LowerTorso.CFrame.UpVector * 2)
                            ) -- emulate head pos

                            if plr_screen ~= -1 then
                                local mag =
                                    (plr_char.LowerTorso.Position - local_player.Character.LowerTorso.Position).Magnitude
                                ...
                                if aimsp_settings.use_esp then
                                    ...
                                    if aimsp_settings.tracers then
                                        utility.update_drawing(
                                            ...
                                            plr_char:GetDebugId(),
                                            {
                                                ...
                                                Thickness = 1,
                                                Color = (aimsp_settings.use_rainbow and utility.get_rainbow()) or
                                    ...
                                    if aimsp_settings.box then
                                        local corners = utility.get_part_corners(plr_char.LowerTorso)
                                        ...
                                        if
                                            (point_a_scr ~= -1) and (point_b_scr ~= -1) and (point_c_scr ~= -1) and
                                                (point_d_scr ~= -1)
                                         then
                                            utility.update_drawing(
                                                objects.quads,
                                    ...
                                    if aimsp_settings.health then
                                        ...
                                        plr_info =
                                            (hum and
                                            plr_info ..
                                                ("[" .. tostring(hum.Health) .. "/" .. tostring(hum.MaxHealth) .. "]")) or
                                            plr_info
                                    end

                                    if plr_info ~= "" then
                                        local cam_mag =
                                            (camera.CFrame.Position - plr_char.LowerTorso.CFrame.Position).Magnitude /
                                            20

                                        local scr_pos =
                                            utility.to_screen(
                                            plr_char.LowerTorso.Position + vector3_new(0, 4, 0) +
                                                (plr_char.LowerTorso.CFrame.UpVector * cam_mag)
                                        )
                                ...
                                if aimsp_settings.prefer.looking_at_you then
                                    local look_vector =
                                        plr_char.LowerTorso.Position + (plr_char.LowerTorso.CFrame.LookVector * mag)

                                    local look_vector_lp_head_dist =
                                        (look_vector - local_player.Character.LowerTorso.Position).Magnitude
                                ...
                                if screen_pos ~= -1 then
                                    if
                                        utility.is_inside_fov(screen_pos) and
                                            utility.is_part_visible(local_player.Character.Humanoid, part)
                                     then
                                        last = {
                                            scr_pos = screen_pos,
                                            obj = part
